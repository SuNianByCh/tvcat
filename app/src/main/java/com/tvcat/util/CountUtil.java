...
/**
 * Created by sunian on 2018/4/10.
 * 计数器类
 */
...
public class CountUtil {
    ...
    public void countBackMainThread(int timeSends, Consumer<Long> consumer) {
        if (consumer == null)
            return;
        Disposable subscribe = Observable.intervalRange(1, timeSends, 0, 1, TimeUnit.SECONDS).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .filter(aLong -> {
