...
public class StateButton extends AppCompatButton {
    ...
    private boolean mRoundLeft;
    private boolean mRoundRiht;
    ...
    private void setup(AttributeSet attrs) {
        ...
        setStroke();
        mLeftTop = a.getDimension(R.styleable.StateButton_conerLeft_top, 0);
        mLeftBottom = a.getDimension(R.styleable.StateButton_conerLeft_bottom, 0);
        mRightTop = a.getDimension(R.styleable.StateButton_conerRight_top, 0);
        mRightBottom = a.getDimension(R.styleable.StateButton_conerRight_bottom, 0);

        mRoundLeft = a.getBoolean(R.styleable.StateButton_round_left, false);
        mRoundRiht = a.getBoolean(R.styleable.StateButton_round_right, false);

        if (!mRoundLeft && !mRoundRiht && (mLeftTop > 0 || mLeftBottom > 0 || mRightTop > 0 || mRightBottom > 0)) {
            float[] floats = {mLeftTop, mLeftTop, mRightTop, mRightTop, mRightBottom, mRightBottom, mLeftBottom, mLeftBottom};
            setRadius(floats);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setRound(mRound);
        setRoundLeftAndRight(mRoundLeft, mRoundRiht);
    }

    private void setRoundLeftAndRight(boolean mRoundLeft, boolean mRoundRiht) {

        if (!(mRoundLeft || mRoundRiht))
            return;
        ...
        if (mRoundLeft && mRoundRiht) {
            ...
            float[] floats = {height, height, mRightTop, mRightTop, mRightBottom, mRightBottom, height, height};
            setRadius(floats);
        } else if (mRoundRiht) {
            float height = getMeasuredHeight() / 2.0f;

            float[] floats = {mLeftTop, mLeftTop, height, height, height, height, mLeftBottom, mLeftBottom};
            setRadius(floats);
        }
